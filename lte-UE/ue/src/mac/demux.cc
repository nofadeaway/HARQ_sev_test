...
void demux::process_pdu(uint8_t *mac_pdu, uint32_t nof_bytes)/////////////////////////////////////UE 下行解包
{
  // Unpack DLSCH MAC PDU 
...
void demux::process_sch_pdu(srslte::sch_pdu *pdu_msg)//////////////////////////调用
{  
  while(pdu_msg->next()) {
    if (pdu_msg->get()->is_sdu()) {
      // Route logical channel 
      Info("Delivering PDU for lcid=%d, %d bytes\n", pdu_msg->get()->get_sdu_lcid(), pdu_msg->get()->get_payload_size());
      rlc->write_pdu(pdu_msg->get()->get_sdu_lcid(), pdu_msg->get()->get_sdu_ptr(), pdu_msg->get()->get_payload_size()); //////////////rlc     
    } else {
